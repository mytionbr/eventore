...
export const signin = ({user_email, user_password}) => async (dispatch) => {
  ...
  export const unregisterEvent = (event) => async (dispatch,getState) => {
    const {
      userSignin: { userInfo },
    } = getState();

    dispatch({
      type: USER_UNREGISTER_EVENT_REQUEST,
    });
    try {
      event.user_id = userInfo.user_id
      const { data } = await api.unregisterEvent({userInfo,event});
      dispatch({ type: USER_UNREGISTER_EVENT_SUCCESS, payload: data });
    } catch (error) {
      dispatch({
        type:USER_UNREGISTER_EVENT_FAIL,
        payload: error.response.data.message || error.message,
      });
    }
  };